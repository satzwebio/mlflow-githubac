...
The code for both the apps are available in `src/api` and `streamlit_app` already. To build and launch these apps 
  ...
  * Add a  `Dockerfile` in the root of the source code for building FastAPI 
      `Build a fastapi image - 
      docker image build -t fastapi .
      docker image ls
      docker image history api
      docker run -idtP fastapi:latest     # start the conatiner
      docker ps -l
      docker logs 90c95b6d9ca5 
      Login to docker hub
      # tag local image to the reg imag
      docker tag fastapi:latest satzweb/fastapi:latest
      docker push satzweb/fastapi:latest
      `      
  * Add `streamlit_app/Dockerfile` to package and build the Streamlit app  
        `Make sure to update the API_URL as "http://localhost:32768" based on the where model API is exposed.
         Build the streamlit app
         docker image build -t streamlit:v1 .

  * Add `docker-compose.yaml` in the root path to launch both these apps. be sure to provide `API_URL=http://fastapi:8000` in the streamlit app's environment. 
      docker compose build # build the image
      docker compose up -d
         [+] Running 3/3
         ✔ Network house-price-predictor_default        Created                                 0.1s 
         ✔ Container house-price-predictor-fastapi-1    Started               0.9s 
         ✔ Container house-price-predictor-streamlit-1  Started     
      docker compose ps
      docker compose logs
